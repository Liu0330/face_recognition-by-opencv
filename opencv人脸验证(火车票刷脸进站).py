...
def take_face():
    ...
    while True:
        ...
        if isCreate and (num == 1):
            ...
            os.makedirs('./faces/%s' % (file), exist_ok=True)
            isCreate = False
        ...
        if ord('w') == key:
            ...
            cv2.waitKey(42)
            cv2.imwrite('./faces/%s/%d.jpg' % (file,num),
...
def load_faces():
    ...
    for i,l in enumerate(labels):
        print(i,l)
        for j in range(1,6):
            face = cv2.imread('./faces/%s/%d.jpg'%(l,j))
            # print(face)

            face2 = cv2.cvtColor(face,code=cv2.COLOR_BGR2GRAY)
            X.append(face2)
            y.append(i)
    X = np.asarray(X)
    y = np.asarray(y)
    ...
    while True:
        ...
        try:
            for x,y,w,h in faces:
                ...
                gray = cv2.resize(gray,(640,360),interpolation=cv2.INTER_LINEAR)
